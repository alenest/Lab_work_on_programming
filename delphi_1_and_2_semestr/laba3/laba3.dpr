...
begin
 try
  write('введите целое число R=?'); readln(r);
  AddR(NachaloSpiska, KonecSpiska, r);
 except
...
var
  ...
  flag : boolean;
begin
         // сами - поиск и вывод результата
         // для перебора элементов списка используйте цикл
  ListC:=NachaloSpiska;
  flag := False;
  while (ListC<>nil) and (not flag) do   // или while (ListC<>nil) and not flag do
  begin
             // Здесь доступен текущий элемент и информация в нем ListC^.info
    if (ListC^.info mod 2 <> 0)  then
    begin
      writeln('Первый нечётный элемент в списке: ', ListC^.info);
      flag := True;
    end;

    ListC:=ListC^.next;
  end;
  if not flag then writeln('Нечетных элементов в списке нет');
...
{ Для того, чтобы каждый раз при запуске программы генерировалась
  ...
  repeat
    ...
    writeln('Y - просмотр;');
    writeln('U - конец.');
    writeln('K - удалить K-ый элемент');
    writeln('D - удалить последний элемент');
